                  ...
                  const Pathname& pathname)
{
  ...
  {
    {
      ...
      ss << "Error: " << pathname.str() << ": not a 'pingus-level' file";
  ...
  else
  {
    ...
    if (!reader.read_section("head", head))
    {
      {
        std::ostringstream ss;
        ss << "Error: (head) section not found in '" << pathname.str() << "'";
        throw std::runtime_error(ss.str());
      }
    }
    else
    {
      ...
      else
      {
        {
          std::ostringstream ss;
          ss << "Error: (pingus-level head actions) not found in '" << pathname.str() << "'";
          throw std::runtime_error(ss.str());
        }
      }
    }
