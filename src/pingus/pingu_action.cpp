...
PinguAction::move_with_forces ()
{
  ...
  if (std::abs(resultant_force.x) > std::abs(resultant_force.y))
  {
    // Initialise so that we move in whole pixels in x direction and
    // 'fractions' of a pixel in y direction.
    denominator = static_cast<int>(std::abs(resultant_force.x));
    x_inc = denominator;
    y_inc = static_cast<int>(std::abs(resultant_force.y));
  }
  else
  {
    // Initialise so that we move in whole pixels in y direction and
    // 'fractions' of a pixel in x direction.
    denominator = static_cast<int>(std::abs(resultant_force.y));
    x_inc = static_cast<int>(std::abs(resultant_force.x));
    y_inc = denominator;
  }
